...
<script runat="server">
    ...
    Protected Sub busqueda_realizada(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        'ALERTAS PARA EL NO PAGO DE PROMOCION DE FIN AÑO
        ...
        If urbanizacion = "J.LIPARI" Or urbanizacion = "J.LIPARI1" Or urbanizacion = "TERRAZAS DE LIPARI" Or urbanizacion = "Cerro Negro" Or urbanizacion = "Media Luna" Or urbanizacion = "URB. PRADERA DE SAYTU" Then
            Msg1.Text ="LA URBANIZACION: " & urbanizacion & " NO PARTICIPA DE LA PROMOCION DE FIN DE AÑO!"
        End If
        ...
        If urbanizacion = "LAS PALMERAS" Then
            Msg1.Text = "LA URBANIZACION: " & urbanizacion & " DEBE PAGAR IMPUESTOS ANUALES POR OTROS SERVICIOS, SEGÚN LISTADO NOTA No. 03/2022 AREA LEGAL - DIVISION Y PARTICIÓN"
        End If
        ...
        If terrasur.traspaso.reembolso.VerificarReembolsoContrato(ContratoBusqueda1.id_resultado) Then
            permitir_cobrar = False
            Msg1.Text = "NO puede realizar pagos sobre este contrato, debido a que se realizó un Traspaso/Devolución sobre el mismo"
        End If
    ...
    Protected Sub CargarPagos()
        ...
        If ica > 0 Then
            ...
            lbl_mensaje_ica.Text = "El cliente de este contrato adeuda $us " & ica.ToString("F2") & " por concepto de Interés Corriente Acumulado"
        Else
        ...
        If negocio_contrato.CodigoNegocioPorContrato(ContratoDatos1.id_contrato) = "nafibo" Then
            ...
            If DateTime.Now.DayOfWeek = DayOfWeek.Saturday Then
                If DateTime.Now > parametro.ConvertDecimalToDateTime(DateTime.Now, New parametro(par_fuera_hora_sabado_hora).valor) Then
                    Msg2.Text = "EL PAGO SERÁ REALIZADO FUERA DE HORA"
                End If
            Else
            ...
            If ",6966,6967,7053,7377,7531,7920,7987,8656,8828,10118,".Contains("," & coObj.numero & ",") = True Then
                Msg3.Text = "El depósito en caja del BNB de este contrato (" & coObj.numero & ") NO PUEDE SER INFERIOR a la CUOTA MENSUAL ESTABLECIDA, comunicarse con ERLAND ESPINOZA o FELIX NINA para realizar el depósito en caja del BNB"
            End If
        End If
        If contrato_estado_especial.VerificarActivo(ContratoDatos1.id_contrato, 0, "", "correspondencia") Then
            ...
            Next
            'Msg3.Text = "Solo cobrar una cuota mensualmente por instrucción de Armando Balcazar"
        ...
        'lbl_dprs.Text = "Se realizaron " & Num_dprs & " pago(s) con DPR (de Promoción) por un monto de $us " & Monto_dprs.ToString("F2") & " desde el 06/11/2012"
        ...
        If permiso_promocion("periodo_correcto").ToString = "1" Then
            If permiso_promocion("permitir").ToString = "1" Then
                lbl_promocion_si.Text = "El cliente puede acceder a la promoción"
                lbl_promocion_no.Text = ""

                If permiso_promocion("al_dia").ToString = "0" Then
                    lbl_promocion_no.Text = "El contrato no está al día"
                End If
            Else
                lbl_promocion_si.Text = ""
                lbl_promocion_no.Text = "El cliente NO puede acceder a la promoción"

                If permiso_promocion("es_terreno").ToString = "1" Then
                    If permiso_promocion("es_nafibo").ToString = "1" Then
                        lbl_promocion_no.Text &= ": " & "El contrato Nafibo no puede acceder a la promoción"
                    Else
                        If Integer.Parse(permiso_promocion("num_pagos_promo_mes").ToString()) > 0 Then
                            lbl_promocion_no.Text &= ": " & "El cliente ya accedió a la promoción"
                        Else
                            If permiso_promocion("al_dia").ToString = "0" Then
                                lbl_promocion_no.Text &= ": " & "El contrato no está al día"
                            End If
        ...
        'Else
        '    lbl_promocion_si.Text = ""
        '    lbl_promocion_no.Text = "El cliente NO puede acceder a la promoción en las sucursales"
        'End If
        ...
        If DateTime.Now.Date >= DateTime.Parse("01/01/2015") And DateTime.Now.Date <= DateTime.Parse("28/02/2015") Then
            ...
            If es_suizaII > 0 Then
                If pago_impuestos = 0 Then
                    Msg4.Text = "El contrato es de la Suiza II, debe pagar IMPUESTOS de la gestión 2013"
                End If
            End If
        End If
    End Sub
                            ...
                            <tr>
                                <td>
                                    <table class="tableContenido" align="center">
                                        ...
                                        <tr><td align="center"><asp:Label ID="lbl_correspondencia" runat="server" Visible="false" Text="Solo cobrar una cuota mensualmente por instrucción de Armando Balcazar" SkinID="lbl_CajaIngresoPermitido"></asp:Label></td></tr>
                                        ...
                                        <tr>
                                            <td class="tdGrid">
                                                <table align="center" width="100%">
                                                    ...
                                                    <tr>
                                                        <td>
                                                            <asp:Panel ID="panel_liquidacion" runat="server" GroupingText="LIQUIDACIÓN DEL CONTRATO">
                                                                <table align="center" width="100%">
