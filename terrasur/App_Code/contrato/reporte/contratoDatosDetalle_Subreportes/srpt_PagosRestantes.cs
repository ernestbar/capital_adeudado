...
/// <summary>
/// Summary description for srpt_PagosRestantes.
/// </summary>
public class srpt_PagosRestantes : DataDynamics.ActiveReports.ActiveReport3
{
    ...
    public void DatosPagosRestantes(int Id_contrato)
    {
        terrasur.contrato c = new terrasur.contrato(Id_contrato);
        if (c.id_planpago_vigente > 0)
        {
            terrasur.plan_pago pp = new terrasur.plan_pago(c.id_planpago_vigente);
            DataTable tabla = terrasur.contratoReporte.ReportePagosRestantesTotales(c.id_contrato, c.preferencial, c.id_ultimo_pago, Decimal.Parse(pp.cuota_base.ToString()), pp.num_cuotas, pp.fecha_inicio_plan, pp.interes_corriente, pp.seguro, pp.mantenimiento_sus);
            foreach (DataRow fila in tabla.Rows)
            {
                textBox7.Text = fila["num_cuotas"].ToString();
                textBox8.Text = fila["num_cuotas"].ToString();
                textBox9.Text = fila["seguro_meses"].ToString();
                textBox10.Text = fila["mantenimiento_meses"].ToString();
                textBox11.Text = fila["interes_dias"].ToString();
                Decimal monto_pago = Math.Round(Decimal.Parse(fila["monto_pago"].ToString()), 2);
                textBox12.Text = monto_pago.ToString("F2");
                Decimal seguro = Math.Round(Decimal.Parse(fila["seguro"].ToString()), 2);
                textBox13.Text = seguro.ToString("F2");
                Decimal mantenimiento = Math.Round(Decimal.Parse(fila["mantenimiento_sus"].ToString()), 2);
                textBox14.Text = mantenimiento.ToString("F2");
                Decimal interes = Math.Round(Decimal.Parse(fila["interes"].ToString()), 2);
                textBox15.Text = interes.ToString("F2");
            }
        } 
        else
        {
            DataTable tabla = terrasur.contratoReporte.ReportePagosRestantesTotales(c.id_contrato, c.preferencial, c.id_ultimo_pago, Decimal.Parse(c.cuota_base.ToString()), c.num_cuotas, c.fecha_inicio_plan, c.interes_corriente, c.seguro, c.mantenimiento_sus);
            foreach (DataRow fila in tabla.Rows)
            {
                textBox7.Text = fila["num_pago"].ToString();
                textBox8.Text = fila["num_cuotas"].ToString();
                textBox9.Text = fila["seguro_meses"].ToString();
                textBox10.Text = fila["mantenimiento_meses"].ToString();
                textBox11.Text = fila["interes_dias"].ToString();
    ...
    private void srpt_PagosRestantes_ReportStart(object sender, EventArgs e)
    {
	...
	{
        string resourceFileName = "srpt_PagosRestantes.resx";
        System.Resources.ResourceManager resources = Resources.srpt_PagosRestantes.ResourceManager;
        ...
        this.textBox1.Text = "Pagos restantes";
        // 
        // textBox2
        // 
        ...
        this.ReportStart += new System.EventHandler(this.srpt_PagosRestantes_ReportStart);
